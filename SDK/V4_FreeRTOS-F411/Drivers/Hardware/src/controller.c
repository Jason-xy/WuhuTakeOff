...
void Input_Capture_Init(void)
{
    ...
    for (i = 0; i < 2; i++)
    {
        HAL_TIM_IC_Start_IT(&htim1, Channel[i]);
    }
    for (i = 0; i < 2; i++)
    {
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
    ...
    else if (htim->Instance == TIM2)
    {
        for (i = 2; i < 4; i++)
        {
            if (htim->Channel == ActiveChannel[i-2] && Duty[5] > 0.07)
            {
                cap = 1;         //标志是否进行了一次捕获
                switch (Flag[i]) //捕获状态
                {
                case 0:                                                                //第一次捕获到上升沿
                    ...
                    TIM_SET_CAPTUREPOLARITY(htim, Channel[i-2], TIM_ICPOLARITY_FALLING); //设置为下降沿捕获
                    Flag[i]++;                                                         //修改捕获状态
                    break;
                ...
                case 1:                                                      //第一次捕获到下降沿
                    ...
                    //准备进行下一次捕获
                    TIM_RESET_CAPTUREPOLARITY(htim, Channel[i-2]);                      //重置触发条件
                    TIM_SET_CAPTUREPOLARITY(htim, Channel[i-2], TIM_ICPOLARITY_RISING); //设置为下降沿捕获
                    Flag[i] = 0;                                                      //重置捕获状态
                    break;
                }
            }
